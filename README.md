...
原生 SwiftUI App，最初是网页版的移植，后改为完全端上处理：用 Vision 的前景抠图 API 提取拍摄主体（不依赖任何远端 AI），加白色模切边，冲印动画（静置约 5 秒或摇晃手机瞬间显影），最后贴纸落入一个陀螺仪驱动的物理沙盘里滑动碰撞。历史记录纯本地存储，无需账号。
...
| | web | ios |
|---|---|---|
| 抠图/贴纸生成 | 服务端调用豆包图生图模型 | 端上 Vision 前景抠图，无需联网 |
